...
class DatabaseBackend
{
	...
	public function insertUser($username, $password, $realname){
		
		// prepare an insert statement
		$query = $this->database->prepare('INSERT INTO ps5_user (username, password_hash, realname, role) VALUES (:usrname, :pwdhash, :realname, \'client\');');
		...
		// return the new user's ID -- all new users start out as client accounts
		$_SESSION['userID'] = $this->database->lastInsertId();
	...
	// deletes a user from the database. returns true if succeeded, false if failed.
	public function deleteUser($userid){
		
		// prepare a delete statement
		$query = $this->database->prepare('DELETE FROM ps5_user WHERE user_pk = :userid');
	...
	// modifies a user's account role. returns true if succeeded, false if failed.
	public function updateUserRole($userid, $role){
		
		// prepare an update statement
		$query = $this->database->prepare('UPDATE ps5_user SET role = :role WHERE user_pk = :userid');
	...
	public function login($username, $password){
		...
		$query = $this->database->prepare('SELECT user_pk, realname, role FROM ps5_user WHERE username = :usrname AND password_hash = :pwdhash');
		...
		if($userid = $query->fetch()){
			...
			$_SESSION['realname'] = $userid['realname']; 
	...
	// return user id if username exists, 0 if it does not 
	private function getUserID($username){
		
		// prepare a select statement
		$query = $this->database->prepare('SELECT user_pk FROM ps5_user WHERE username = :username');
	...
	// returns resume id if resume name exists for the given user, 0 if it does not
	private function getResumeID($resumename, $userid){
		
		// prepare a select statement
		$query = $this->database->prepare('SELECT resume_pk FROM ps5_resume WHERE name = :resumename AND user_fk = :userid');
		...
		// return true if there was a record with that ID
		if($row = $query->fetch())
			return $row['resume_pk'];
		else
			return 0;
	}
	
	// returns a preview of a resume given its name and its creator's username
	public function previewResumeAsArray($resumename, $username){
		// get user id
		$userid = $this->getUserID($username);
		
		// if no user by that name exists, return false
		if($userid == 0)
			return false;
		
		// get resume id
		$resumeid = $this->getResumeID($resumename, $userid);
		
		// if no resume by that name exists for this user, return false
		if($resumeid == 0)
			return false;
		
		// user exists, and a resume by this name exists for this user, so return it as an array
		return $this->getResumeAsArray($resumeid);
	}
	
	// returns a list of resume clients and admins as an array
	public function getUsers(){

		// prepare a select statement
		$query = $this->database->prepare('SELECT user_pk, username, realname, role FROM ps5_user ORDER BY role ASC');
		
		// get the user's resumes
		$query->execute();
		
		// initialize array to hold the users
		$users = array();
		$userids = array();
		$usernames = array();
		$realnames = array();
		...
		// return true if there was a record with that ID
		while($row = $query->fetch()){
			array_push($userids, $row['user_pk']);
			array_push($usernames, $row['username']);
			array_push($realnames, $row['realname']);
			array_push($roles, $row['role']);
		}
		
		// put the individual arrays into the user array
		array_push($users, $userids);
		array_push($users, $usernames);
		array_push($users, $realnames);
		array_push($users, $roles);
		
		// return the complete array of users
		return $users;
	}
};